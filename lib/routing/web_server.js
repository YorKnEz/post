    ...
    constructor(path, routes, options) {
        ...
        if (options.envLocation && Object.keys(options.env).length > 0) {
            try {
                ...
                console.log(`${options.envLocation} bootstrapped`)
            } catch (e) {
                console.error(
                    `Error bootstrapping ${options.openapi.path}: ${e}`
                )
                return
            }
        }
        ...
        if (options.openapi) {
            try {
                const data = fs.readFileSync(options.openapi.path, 'utf8')
                const result = data.replace(/url: '.*'/, `url: '${options.openapi.url}/docs/docs.yaml'`)
                fs.writeFileSync(options.openapi.path, result, 'utf8')

                console.log(`${options.openapi.path} bootstrapped`)
            } catch (e) {
                console.error(
                    `Error bootstrapping ${options.openapi.path}: ${e}`
                )
