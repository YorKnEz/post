...
export default class Router extends BaseRouter {
    ...
    async handle(req, res) {
        ...
        let router = this.router.get(req.method.toLowerCase())

        // if an OPTIONS request is made, guide the request to the handler of the original method
        // and if a CORS middleware is present, it will take care of it, otherwise, CORS will fire
        if (req.method == 'OPTIONS') {
            router = this.router.get(
                req.headers['access-control-request-method'].toLowerCase()
            )
        }
        ...
        for (const [url, handlers] of router) {
            for (const handler of handlers) {
                if (req.url.startsWith(url)) {
                    ...
                    const params = url.match(/:[\w\.\-_]+/g) // get path params
